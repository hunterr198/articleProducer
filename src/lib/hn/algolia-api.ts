...
export async function fetchAllRecentStories(hoursBack = 24, minPoints = 5): Promise<HNStory[]> {
  const since = Math.floor(Date.now() / 1000) - hoursBack * 60 * 60;

  const res = await fetch(
    `${BASE_URL}/search?tags=story&numericFilters=points%3E${minPoints},created_at_i%3E${since}&hitsPerPage=500`
  );
